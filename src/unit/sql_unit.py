...
class SqlUnit:
    @staticmethod
    def initAnimeInfoSql(dataPath: str):
        if os.path.exists(f'{dataPath}/database/animeInfo.db'):
            return
        connection = sqlite3.connect(f'{dataPath}/database/animeInfo.db')
    ...
    @staticmethod
    def getAllDataFromAnimeInfo(dataPath: str):
        connection = sqlite3.connect(f'{dataPath}/database/animeInfo.db')
        cursor = connection.cursor()
    ...
    @staticmethod
    def getAnimeInfoByBangumiId(dataPath: str, bangumiId: int):
        connection = sqlite3.connect(f'{dataPath}/database/animeInfo.db')
        cursor = connection.cursor()
        cursor.execute(
    ...
    @staticmethod
    def getAnimeInfoByAnimeName(dataPath: str, animeName: str):
        connection = sqlite3.connect(f'{dataPath}/database/animeInfo.db')
        cursor = connection.cursor()
    ...
    @staticmethod
    def insertSqlNewLineInAnimeInfo(dataPath: str, bangumiId: int, animeName: str,
